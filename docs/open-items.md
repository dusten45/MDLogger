...
| --- | ----------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| 1   | Qt 자동 UI 클릭 테스트                                | 단계 1, 6 | **완료(2026-08-10 정리 세션)**: 자동 클릭 테스트 12개 + 소유자 수동 확인('버튼 몇 개 눌러 확인, UI·텍스트·아이콘 정상')                    | 지속 회귀 관리          |
...
- **완료(2026-08-10 정리 세션)**: 자동 클릭 테스트 12개(`test_ui_flow_clicks.py` 8 + `test_account_ui_clicks.py` 4, 전체 pytest 261 passed)와 소유자 수동 확인(핵심 기록·통계·편집/삭제·계정 다이얼로그 버튼 클릭, UI·텍스트·아이콘 정상)으로 해소함.
- **남은 꼬리**: 로그인 제출·서버 동기화 등 **네트워크/세션 의존 흐름**은 mock 단위 테스트가 담당하고, 실제 hosted 서버 연동 확인은 ②/④(소유자 환경·단계 12) 몫으로 이 항목(1) 범위 밖. 마이그레이션/백업은 확인 다이얼로그가 없어(백업 내부 전용) 시작 경로 통합으로 대신함.
