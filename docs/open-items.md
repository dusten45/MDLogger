...
| --- | ----------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| 1   | Qt 자동 UI 클릭 테스트                                | 단계 1, 6 | 클릭 테스트 추가(2026-08-10): 저장·취소·유효성, 통계 창·편집/삭제 다이얼로그 흐름 포함 7개                                                 | 지속 회귀 관리          |
...
- **현재 상태**: `tests/test_ui_flow_clicks.py` 추가(2026-08-10). offscreen에서 `QTest.mouseClick`으로 실제 버튼을 클릭해 ① 화면1(결과 선택) → 화면2(상세 입력) → 저장/취소/유효성 검증, ② 통계 창 열기·요약/기록 렌더링, ③ 기록 편집(저장·결과 토글·취소), ④ 삭제 확인 흐름을 구동한다(총 7개). 편집/삭제는 모달(`exec()`)이므로 `QTimer.singleShot`으로 이벤트 루프 안 상호작용을 예약한다. 기존 검증(offscreen smoke·mock 단위 테스트)은 계속 유지한다.
- **해소 예상 위치**: 핵심 저장/취소/검증 흐름과 통계 창·편집/삭제 다이얼로그 흐름은 추가됨. 계정·동기화 UI, 마이그레이션·백업 대화상자 등은 단계 12 통합 시 지속 보강.
