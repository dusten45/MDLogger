...
- [x] 모든 설정이 한 창에서 범주별로 관리된다.
- [x] 설정이 재시작 후 복원된다.
- [x] 저사양 모드가 애니메이션과 고비용 효과를 실제로 줄인다.
- [x] 시스템/밝은/어두운 테마를 선택할 수 있다.
- [x] 글자 크기를 조절해도 주요 화면이 잘리지 않는다.
- [x] 메모를 꺼도 기존 데이터가 삭제되지 않는다.
- [x] 계정 작업이 기존 인증·프로필 경계를 깨지 않고 설정 창에서 수행된다.
- [x] 설정 초기화가 경기와 계정 데이터를 건드리지 않는다.
- [x] 접근성, 테마, DPI, 성능 관련 테스트가 통과한다.
...
**▶ 확정(2단계):** 입력과 기록 열을 모두 숨기되 데이터와 내보내기는 보존한다(권장안). 기존 메모는 DB·서버에서 삭제하지 않는다.
...
**▶ 확정(2단계):** 1(시스템/밝음/어두움)과 2(검증된 강조색 프리셋)만 제공한다. 3(모든 색 자유 지정)은 초기 범위에서 제외한다.
...
**▶ 확정(2단계):** 기본값은 모두 기기 로컬(①). 별도 "설정 동기화" 기능(서버 업로드/다운로드)을 제공하되, 동기화 대상은 취향 설정(테마·강조색·메모 사용·기본 모드 등)만(② 일부만 동기화). 기기 특성 설정(글자 크기·저사양 모드·애니메이션 감소 등)은 동기화에서 하드 차단한다.
