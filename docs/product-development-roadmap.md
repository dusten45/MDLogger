...
> 상세 확정값은 `docs/phase1-ranked-implementation-spec.md` 참조 (코드 구현은 미진행).
...
- [ ] 점수전과 랭크전을 메인 화면에서 선택할 수 있다.
- [ ] 선택한 모드가 모든 저장·수정·삭제·동기화 경로에서 보존된다.
- [ ] 랭크 전후 상태가 기록된다.
- [ ] 기록과 통계를 모드별로 필터링할 수 있다.
- [ ] 점수전과 랭크전 그래프가 의미에 맞게 분리된다.
- [ ] 기존 기록과 내보내기 데이터가 손실되지 않는다.
- [ ] 데스크톱 구버전과의 동기화 호환 정책이 검증되었다.
- [ ] 관련 단위·통합·UI 테스트가 통과한다.
