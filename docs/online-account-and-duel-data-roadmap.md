...
- 메인 결과 화면 상단에 낮은 우선순위의 `프로필 · 인증/오프라인 상태 · 로컬 저장` 텍스트와 계정 진입 버튼을 추가했다. 계정 대화상자는 로그인/다른 계정 전환, 로그아웃, 장치 관리, 모든 기기 로그아웃, 내 데이터 내보내기, 계정 삭제와 현재 로컬 저장 상태를 제공한다. (2026-08-13 갱신: 로그아웃은 게스트 복귀가 아니라 로그인 창으로 돌아가며, 로그인 창에서 다시 로그인하거나 게스트로 계속할 수 있다. 버튼 문구도 "로그아웃"으로 변경했다.)
...
- **실제 세션 폐기(Auth Admin 로그아웃)**: `revoke_all_devices`/`revoke_device` RPC는 장치 행만 제거하고 활성 세션·refresh token은 폐기하지 않았다(D-6).
    - **클라이언트**(2026-08-13): "모든 기기에서 로그아웃" 성공 시 **자신의 로컬 refresh token을 즉시 제거하고 세션을 종료**해 잔존 토큰 재사용을 막는다(`SessionManager.sign_out_all_devices`). 이후 로그인 창으로 돌아가며, 남은 기기 수를 알려주는 작은 알림을 표시한다.
    - **서버**(2026-08-13, Edge Function `revoke-sessions`): Auth Admin(service-role) 경로로 요청자 본인의 **모든 세션·refresh token을 실제로 폐기**(`admin.auth.admin.signOut(token, scope='global')`)하고 장치 행도 정리한다. 클라이언트의 `sign_out_all_devices`는 이제 이 Edge Function을 호출한다.
    - (2026-08-13) **단일 기기 해제 기능 제거**: GoTrue 세션이 기기 단위가 아니라 "그 기기만" 조준해서 로그아웃할 수 없으므로, 헷갈리지 않도록 **단일 기기 해제(장치 관리) UI를 제거**했다. 로그아웃은 "현재 PC에서 로그아웃"과 "모든 기기에서 로그아웃" 두 가지만 남는다.
...
- [x] 실제 세션 폐기(D-6) — 완료 (2026-08-13):
      로아웃을 "현재 PC에서 로그아웃"·"모든 기기에서 로그아웃"으로 재설계(signed_out 영속화·로그인 창 복귀·단일 기기 해제 제거)하고, `revoke-sessions`
      Edge Function(Auth Admin)으로 모든 세션·refresh token 실제 폐기. 단일 기기 해제는 제거. 서버 배포·배포 후 실동작 확인은 소유자 작업으로 남김.
...
| 계정 B 로그아웃 후 로그인 창 복귀 | 로그인 창 표시, 기존 guest 기록 유지 (게스트는 로그인 창에서 선택) |
