...
describe("WebSettings", () => {
    ...
    it("손상된 단일 필드를 기본값으로 대체한다", () => {
        expect(parseSettings({ theme_mode: "bogus" }).theme_mode).toBe(
            "system",
        );
        expect(parseSettings({ accent_color: "bogus" }).accent_color).toBe(
            "blue",
        );
        ...
        expect(
            parseSettings({ score_input_mode: "bogus" }).score_input_mode,
        ).toBe("delta");
    });
    ...
    it("직렬화/역직렬화 왕복이 일치한다", () => {
        ...
        const roundTripped = parseSettings(
            JSON.parse(serializeSettings(settings)),
        );
        expect(roundTripped).toEqual(settings);
    });

    it("PREFERENCE_KEYS/DEVICE_KEYS가 웹 설정 모델과 일치한다", () => {
        ...
        expect(DEVICE_KEYS).toEqual(["font_scale"]);
    });
});
