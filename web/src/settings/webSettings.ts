...
export type ReduceMotion = "system" | "off" | "on";
...
export function parseSettings(data: unknown): WebSettings {
  ...
  return {
    ...
    low_spec_mode: parseBool(source.low_spec_mode, false),
    reduce_motion: parseReduceMotion(source.reduce_motion),
...
/** 저사양 모드 on이면 애니메이션을 강제로 끈다 (데스크톱 `effective_reduce_motion`). */
export function effectiveReduceMotion(settings: WebSettings): boolean {
  return settings.reduce_motion === "on" || settings.low_spec_mode;
}
  ...
  load(): WebSettings {
    ...
    try {
      ...
      if (
        ...
        data !== null &&
        !Array.isArray(data) &&
        (data as Record<string, unknown>).schema_version !== SCHEMA_VERSION
      ) {
        return { ...DEFAULT_SETTINGS };
      }
...
function parseReduceMotion(value: unknown): ReduceMotion {
  if (value === "system" || value === "off" || value === "on") {
    return value;
  }
  return "system";
}
