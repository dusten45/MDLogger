-- 0024_guest_ingest_v2.sql
-- 1단계 payload v2 — guest ingest DB 계약을 v2로 올린다 (spec §4.5).
--
-- 클라이언트 `GUEST_INGEST_PAYLOAD_VERSION`과 Edge Function `PAYLOAD_VERSION`을
-- 2로 올리므로, 최종 집계 경계인 `analytics.ingest_guest_batch`도 v2만 허용한다.
-- 본문은 0013(환경 version 미등록 검증 포함)과 동일하고 payload check만 변경한다.
-- `public.ingest_guest_batch` wrapper(service_role 전용)는 시그니처가 같아 그대로다.
