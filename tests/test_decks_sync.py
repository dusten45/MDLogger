...
def test_sync_noop_when_url_empty(tmp_path, monkeypatch):
    ...
    called = []
    monkeypatch.setattr(sync, "_fetch_remote", lambda *a: called.append(a) or (None, None))
...
def test_sync_304_leaves_file_untouched(tmp_path, monkeypatch):
    ...
    sync.sync_decks(url="http://x", decks_path=decks_path, state_path=tmp_path / "state.json")
...
def test_sync_200_merges_and_preserves_local(tmp_path, monkeypatch):
    _no_real_data_dir(monkeypatch)
    decks_path = tmp_path / "decks.json"
    decks_path.write_text(json.dumps(["로컬전용", OTHER], ensure_ascii=False), encoding="utf-8")
    monkeypatch.setattr(sync, "_fetch_remote", lambda url, etag: (["원격A", "원격B"], "etag1"))
    sync.sync_decks(url="http://x", decks_path=decks_path, state_path=tmp_path / "state.json")
...
def test_sync_writes_state_etag(tmp_path, monkeypatch):
    ...
    sync.sync_decks(url="http://x", decks_path=tmp_path / "decks.json", state_path=state_path)
